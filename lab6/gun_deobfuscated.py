...
class target():
    ...
    def move(self, flag):
        ...
        if self.y + self.r >= 500 or self.y - self.r <= 100:
            self.vy = -self.vy
            self.set_coords()

        if self.x + self.r >= 780 or self.x - self.r <= 300:
            self.vx = -self.vx
            self.set_coords()


"""
Класс target_2 описывает 2ую цель. 
"""
...
def new_game(event=''):
    global gun, t1, t2, screen1, balls, bullet, flag1, flag2
    t1.new_target()
    t2.new_target()
    bullet = 0
    balls = []
